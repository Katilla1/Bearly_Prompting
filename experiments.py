...
from vec2text.models import (
    ...
    InversionFromMultipleLogitsModel,
    InversionFromOutputModel,
    InversionFromJailbreakModel,
    T5SparseEncoder
)
...
class Experiment(abc.ABC):
    ...
    def _load_train_dataset_uncached(
        ...
        print("use_less_data", data_args.use_less_data)
...
class InversionFromLogitsExperiment(InversionExperiment):
    ...
    def load_model(self) -> transformers.PreTrainedModel:
        ...
        elif self.training_args.experiment == "inversion_from_multiple_logits":
            return InversionFromMultipleLogitsModel(config=self.config)
        else:
...
class InversionFromOutputExperiment(InversionExperiment):
    def load_model(self) -> transformers.PreTrainedModel:
        return InversionFromOutputModel(config=self.config)


class InversionFromOutputSparseExperiment(InversionExperiment):
    @property
    def _wandb_exp_name(self) -> str:
        try:
            name = self.exp_name
        except:
            name = 'inversion-from-four-sparse-9000'
        print("Exp name is", name)
        return name
    
    @property
    def _wandb_project_name(self) -> str:
        return "InversionFromOutputSparseExperiment"
    
    def load_model(self) -> transformers.PreTrainedModel:
        return T5SparseEncoder(config=self.config)

class InversionFromJailbreakExperiment(InversionExperiment):
    @property
    def trainer_cls(self):
        class JailbreakPromptTrainer(vec2text.trainers.InversionFromLogitsTrainer):
            def prediction_step(self, *args, **kwargs):
                return None, None, None
        return JailbreakPromptTrainer

    def load_model(self) -> transformers.PreTrainedModel:
        return InversionFromJailbreakModel(config=self.config)
...
EXPERIMENT_CLS_MAP = {
    ...
    "inversion_from_multiple_logits": InversionFromLogitsExperiment,
    "inversion_from_output": InversionFromOutputExperiment,
    "inversion_from_output_sparse": InversionFromOutputSparseExperiment,
    "inversion_from_jailbreak": InversionFromJailbreakExperiment,
